...
def make_config_for_class(cls):
    ...
    defaults = list(reversed(arg_spec.defaults if arg_spec.defaults else []))
    config = {}
    default = None
    for argument in reversed(arguments):
        default, config = update_config(defaults, default, config, argument)
    return (path, config)

def update_config(defaults, default, config, argument):
    if defaults:
