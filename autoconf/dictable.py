...
def nd_array_as_dict(obj: np.ndarray) -> dict:
    ...
    return {'type': 'ndarray', 'array': obj.tolist(), 'dtype': np_type_map.get(np_type, np_type)}
...
def to_dict(obj):
    if isinstance(obj, (int, float, str, bool, type(None))):
        ...
        return handle_dict_attr(obj)
    elif is_array(obj):
        return handle_array(obj)
    elif isinstance(obj, Path):
        return {'type': 'path', 'path': str(obj)}
    else:
        return handle_other_types(obj)

def handle_dict_attr(obj):
    try:
        return obj.dict()
...
def handle_other_types(obj):
    if inspect.isclass(obj):
        return {'type': 'type', 'class_path': get_class_path(obj)}
    elif isinstance(obj, list):
        return {'type': 'list', 'values': list(map(to_dict, obj))}
    elif isinstance(obj, dict):
        return {'type': 'dict', 'arguments': {key: to_dict(value) for key, value in obj.items()}}
...
def instance_as_dict(obj):
    ...
    try:
        ...
        pass
    argument_dict = {arg: getattr(obj, arg) for arg in arguments if hasattr(obj, arg) and (not inspect.ismethod(getattr(obj, arg)))}
    try:
        for field in obj.__nullify_fields__:
            argument_dict[field] = None
    except (AttributeError, TypeError):
        pass
    return {'type': 'instance', 'class_path': get_class_path(obj.__class__), 'arguments': {key: to_dict(value) for key, value in argument_dict.items()}}
__parsers = {'ndarray': nd_array_from_dict}
...
def from_dict(dictionary, **kwargs):
    ...
    return cls(**{name: from_dict(value, **kwargs) for name, value in dictionary['arguments'].items()})
