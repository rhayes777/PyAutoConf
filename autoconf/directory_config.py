...
class RecursiveConfig(AbstractConfig):
    ...
    def keys(self):
        try:
            return [path.split('.')[0] for path in os.listdir(self.path) if all([path != 'priors', len(path.split('.')[0]) != 0, os.path.isdir(f'{self.path}/{path}') or path.endswith('.ini') or path.endswith('.yaml') or path.endswith('.yml')])]
